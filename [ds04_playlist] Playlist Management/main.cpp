...
class list
{
  ...
  public:
    ...
    void reorder(int pos,std::vector<int> selected) {
        if (mSize==0) return;
        node *insert_position=mHeader->next;
        while (pos--) insert_position=insert_position->next;
        ...
        for (int &x:selected){
            ...
            node *tmp=i->prev;
            //move the one of index x to pos
            i->prev->next=i->next;
            i->next->prev=i->prev;
            insert_position->prev->next=i;
            i->prev=insert_position->prev;
            insert_position->prev=i;
            i->next=insert_position;
            i=tmp;
        }
    }
